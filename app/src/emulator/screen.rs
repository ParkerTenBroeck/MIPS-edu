use eframe::{
    egui,
    emath::Rot2,
    epaint::{pos2, Color32, ColorImage, Mesh, Rect, Shape, TextureHandle, Vec2},
};
    ...
    pub fn draw(
        ...
        match self {
            ...
            Layer::TileMap {
                ...
                let rotation: [Rot2; 4] = [
                    Rot2::IDENTITY,
                    Rot2::from_angle(std::f32::consts::PI / 2.0),
                    Rot2::IDENTITY,
                    Rot2::from_angle(std::f32::consts::PI * 3.0 / 2.0),
                ];

                let scroll_x_offset =
                    (scroll[0] % 8) as f32 * display_area.width() / (tiles_x * 8) as f32;
                let scroll_y_offset =
                    (scroll[1] % 8) as f32 * display_area.height() / (tiles_y * 8) as f32;
                ...
                for x in (scroll[0] / 8)..=(tiles_x + (scroll[0] / 8)) {
                    for y in (scroll[1] / 8)..=(tiles_y + (scroll[1] / 8)) {
                        let index = (((x as usize).wrapping_sub(1)) % tiles_x_y[1] as usize)
                            + ((y as usize).wrapping_sub(1) % tiles_x_y[1] as usize)
                                * tiles_x_y[0] as usize;
                    ...
                    if sprite.rot % 256 == 0 {
                        ...
                        mesh.add_rect_with_uv(
                            ...
                            Rect::from_min_max(
                                ...
                                pos2(
                                    (sprite_width + sprite.sp_pos[0] as f32)
                                        / sprite_text.size()[0] as f32,
                                    (sprite_height + sprite.sp_pos[1] as f32)
                                        / sprite_text.size()[1] as f32,
                                ),
                            ),
                            Color32::from_rgba_unmultiplied(
                                sprite.tint[0],
                        ...
                        let bounding = Rect::from_center_size(
                            center,
                            Vec2::new(
                                new_width as f32 / pixels_per_point_x,
                                new_height as f32 / pixels_per_point_y,
                            ),
                        );
                        ...
                        let mut rect = Rect::from_min_max(
                            bounding.min,
                            pos2(bounding.min.x + x_step, bounding.min.y + y_step),
                        );
                        ...
                        for y in 0..new_height {
                            ...
                            if range.contains(&rect.min.y) || range.contains(&rect.max.y) {
                                ...
                                for x in 0..new_width {
                                    ...
                                    let index = (x as isize + sprite.sp_pos[0] as isize) as usize
                                        % sprite_text.size()[0]
                                    ...
                                    if x > sprite.x_size as usize - 1
                                        ...
                                        || {
                                            ...
                                            !(range.contains(&rect.min.x)
                                                || range.contains(&rect.max.x))
                                        }
                                    {
                                    } else {
