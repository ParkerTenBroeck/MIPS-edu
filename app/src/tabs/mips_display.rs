...
use crate::{emulator::{screen::{Screen, Layer}, handlers::{CPUAccessInfo}}, platform::{sync::PlatSpecificLocking}};
...
impl Tab for MipsDisplay{
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        self.screen.onscreen_resolution = if let Layer::BitMapExpand(texture) = &self.screen.layers[0]{
            texture.size()
        }else{
            [128, 128]
        };
        ...
        let mut calc_rect = Vec2::new(self.screen.onscreen_resolution[0] as f32, self.screen.onscreen_resolution[1] as f32).normalized();
        ...
        if let Option::Some(pos) = resp.hover_pos(){
            if calc_rect.contains(pos){
                ...
                let middle = ui.ctx().input().pointer.button_down(egui::PointerButton::Middle);
