...
pub fn start_thread(f: impl FnOnce() + Send + 'static) -> Result<(), Box<dyn Error>> {
    
    #[cfg(not(target_arch = "wasm32"))]
    {
        std::thread::Builder::new().stack_size(32 * 1024 * 1024).spawn(f)?;
        //std::thread::Builder::new()
        Ok(())
    }
    #[cfg(target_arch = "wasm32")]
    {
        match pool::WorkerPool::new(1){
            Ok(ok) => match ok.run(f){
                Ok(_) => {
                    Ok(())
                },
                Err(err) => {
